...
def convert_wav_16bit(src_path, dst_path):
    """
    Converts any WAV (float64, float32, int32, int16) to 16-bit PCM WAV safely.
    """
    # Read audio using soundfile (supports float64)
    ...
    if data.dtype in [np.float64, np.float32]:
        ...
        # downscale int32 to int16
        data = (data >> (data.dtype.itemsize*8 - 16)).astype(np.int16)
    else:
        data = data.astype(np.int16)

    # Save as 16-bit WAV using PyDub
    audio = AudioSegment(
        data.tobytes(),
...
def prepare_emovdb(
    ...
    # wav ofiles f EmoV-DB are saved in double instead of int16
    ...
    for path in all_paths:
        ...
        if "converted" not in str(path):
            convert_wav_16bit(path, convert_path)
...
def get_emotion(wav_path):
    ...
    if "amused" in wav_path:
        return "happy"
    if "disgust" in wav_path:
        return "disgust"
    if "anger" in wav_path:
        return "angry"
    if "neutral" in wav_path:
        return "neutral"
...
def concat_wavs(data_folder, save_json): 
    ...
    for repo in repos_processed:
        ...
        disgust_files = paths.rglob("disgust*.wav")
...
if __name__ == "__main__":
    data_folder = "datasets/EmoV-DB"
    save_json = os.path.join(data_folder, "EMOV-DB.json")
    prepare_emovdb(data_folder, save_json)
