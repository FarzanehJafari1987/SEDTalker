...
def get_emotion(wav_path):
    ...
    if num == "01":
        return "neutral"  # ADDED
    elif num == "02":
        return "neutral"  # ADDED (calm → neutral)
    elif num == "03":
        return "happy"
    elif num == "04":
        return "sad"
...
#         fear_files = paths.rglob("*-*-06-*-*-*-*.wav")
#         disgust_files = paths.rglob("*-*-07-*-*-*-*.wav")
...
def concat_wavs(data_folder, save_json):
    ...
    for repo in repos:
        ...
        while len(emotion_wavs) > 0:
            # Check available resources before choosing combination
            available_combinations = []
            available_probs = []
            
            if len(neutral_wavs) >= 1:
                available_combinations.extend(["neu_emo", "emo_neu"])
                available_probs.extend([0.25, 0.25])
            
            if len(neutral_wavs) >= 2:
                available_combinations.append("neu_emo_neu")
                available_probs.append(0.25)
            
            available_combinations.append("emo_emo")
            available_probs.append(0.25)
            ...
            combination = np.random.choice(available_combinations, p=available_probs)
...
if __name__ == "__main__":
    ...
    prepare_ravdess(data_folder, save_json)